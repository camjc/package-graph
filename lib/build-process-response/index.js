...
const buildGetEdges = ({ externalModule, organization, packageVersionsByName }) =>
  compose(
    join("\n"),
    uniq,
    flatten,
    map(buildGetStringFromDependencyRelationship(packageVersionsByName)),
    filter(({ dependency }) => includes(organization)(dependency) || dependency === externalModule),
    flatten,
...
const buildProcessResponse = ({ externalModule, organization }) => body => {
